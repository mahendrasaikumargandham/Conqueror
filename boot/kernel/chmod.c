...
static bool
mode_changed (int dir_fd, char const *file, char const *file_full_name, mode_t old_mode, mode_t new_mode)
{
  if (new_mode & (S_ISUID | S_ISGID | S_ISVTX))
    {
      ...
      if (fstatat (dir_fd, file, &new_stats, 0) != 0)
        {
          if (! force_silent)
            error (0, errno, _("getting new attributes of %s"), quoteaf (file_full_name));
          return false;
        }
...
static void
describe_change (char const *file, mode_t old_mode, mode_t mode, enum Change_status changed)
{
  char perms[12];		
  char old_perms[12];
  char const *fmt;
...
process_file (FTS *fts, FTSENT *ent)
{
      ...
      if (! S_ISLNK (old_mode))
        {
          if (chmodat (fts->fts_cwd_fd, file, new_mode) == 0)
            ...
            {
              if (! force_silent)
                error (0, errno, _("changing permissions of %s"), quoteaf (file_full_name));
              ok = false;
            }
        }
    }
  ...
  if (verbosity != V_off)
    {
      bool changed = (chmod_succeeded && mode_changed (fts->fts_cwd_fd, file, file_full_name, old_mode, new_mode));

      if (changed || verbosity == V_high)
        {
          enum Change_status ch_status =
            (!ok ? CH_FAILED : !chmod_succeeded ? CH_NOT_APPLIED : !changed ? CH_NO_CHANGE_REQUESTED : CH_SUCCEEDED);
      ...
      if (new_mode & ~naively_expected_mode)
        {
          ...
          error (0, 0, _("%s: new permissions are %s, not %s"), quotef (file_full_name), new_perms + 1, naively_expected_perms + 1);
          ok = false;
        }
    }
  ...
  if (reference_file)
    {
      ...
      if (!change)
        die (EXIT_FAILURE, errno, _("failed to get attributes of %s"), quoteaf (reference_file));
    }
  else
    {
