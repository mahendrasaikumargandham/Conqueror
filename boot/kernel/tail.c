...
static void ATTRIBUTE_NORETURN die_pipe (void)
{
  raise (SIGPIPE);
  exit (EXIT_FAILURE);
}
      ...
      if (!any_input)
        {
          if (writer_is_dead)
            break;
          writer_is_dead = (pid != 0 && kill (pid, 0) != 0 && errno != EPERM);
...
static bool any_remote_file (const struct File_spec *f, size_t n_files)
{
  for (size_t i = 0; i < n_files; i++)
