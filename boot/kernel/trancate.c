...
static bool do_ftruncate (int fd, char const *fname, off_t ssize, off_t rsize, rel_mode_t rel_mode)
{
  struct stat sb;
  off_t nsize;
  ...
  if (block_mode)
    {
      ...
      if (INT_MULTIPLY_WRAPV (ssize, blksize, &ssize))
        {
          error (0, 0, _("overflow in %" PRIdMAX " * %" PRIdPTR " byte blocks for file %s"), ssize0, blksize, quoteaf (fname));
          return false;
        }
    }
  if (rel_mode)
          ...
          if (usable_st_size (&sb))
            {
              ...
              if (fsize < 0)
                {
                  error (0, 0, _("%s has unusable, apparently negative size"), quoteaf (fname));
                  return false;
                }
            }
          else
      ...
      if (rel_mode == rm_min)
      ...
      else
        {
          ...
          if (INT_ADD_WRAPV (fsize, ssize, &nsize))
            {
              error (0, 0, _("overflow extending size of file %s"), quoteaf (fname));
              return false;
            }
        }
    }
  ...
  if (ftruncate (fd, nsize) == -1)
    {
      error (0, errno, _("failed to truncate %s at %" PRIdMAX " bytes"), quoteaf (fname), (intmax_t) nsize);
      return false;
    }
  return true;
}
  ...
  while ((c = getopt_long (argc, argv, "cor:s:", longopts, NULL)) != -1)
    {
      switch (c)
        ...
        case 's':
          ...
          size = xdectoimax (optarg, OFF_T_MIN, OFF_T_MAX, "EgGkKmMPtTYZ0", _("Invalid number"), 0);
