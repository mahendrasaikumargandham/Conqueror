...
static void check_order (struct linebuffer const *prev, struct linebuffer const *current, int whatfile)
{
      ...
      if (!issued_disorder_warning[whatfile - 1])
        {
          ...
          if (hard_LC_COLLATE)
            order = xmemcoll (prev->buffer, prev->length - 1, current->buffer, current->length - 1);
          else
            order = memcmp2 (prev->buffer, prev->length - 1, current->buffer, current->length - 1);
  ...
  for (i = 0; i < 2; i++)
    {
      ...
      thisline[i] = readlinebuffer_delim (all_line[i][alt[i][0]], streams[i], delim);
      if (ferror (streams[i]))
        die (EXIT_FAILURE, errno, "%s", quotef (infiles[i]));
    }
  ...
  if (total_option)
    {
      ...
      printf ("%s%s%s%s%s%s%s%c", umaxtostr (total[0], buf1), col_sep, umaxtostr (total[1], buf2), col_sep, umaxtostr (total[2], buf3), col_sep, _("total"), delim);
    }
}
