...
main (int argc, char **argv)
{
  ...
  while ((optc = getopt_long (argc, argv, "HLPRcfhv", long_options, NULL)) != -1)
    {
      switch (optc)
        ...
        case FROM_OPTION:
          {
            char const *e = parse_user_spec (optarg, &required_uid, &required_gid, NULL, NULL);
            if (e)
  ...
  if (reference_file)
    ...
    {
      char const *e = parse_user_spec (argv[optind], &uid, &gid, &chopt.user_name, &chopt.group_name);
      if (e)
  ...
  ok = chown_files (argv + optind, bit_flags, uid, gid, required_uid, required_gid, &chopt);
