  ...
  if (specified_context == NULL)
    {
      ...
      if (status < 0 && errno != ENODATA)
        {
          error (0, errno, _("failed to get security context of %s"), quoteaf (file));
          return 1;
        }
  ...
  if (file_context == NULL || ! STREQ (context_string, file_context))
    {
      ...
      if (fail)
        {
          errors = 1;
          error (0, errno, _("failed to change context of %s to %s"), quoteaf_n (0, file), quote_n (1, context_string));
        }
    }
  if (specified_context == NULL)
    {
  ...
  if (recurse)
    {
      if (bit_flags == FTS_PHYSICAL)
        {
          if (dereference == 1)
            die (EXIT_FAILURE, 0, _("-R --dereference requires either -H or -L"));
  ...
  if (reference_file)
    {
      ...
      if (getfilecon (reference_file, &ref_context) < 0)
        die (EXIT_FAILURE, errno, _("failed to get security context of %s"), quoteaf (reference_file));
  ...
  else
    {
      ...
      if (0 < is_selinux_enabled () && security_check_context (specified_context) < 0)
        die (EXIT_FAILURE, errno, _("invalid context: %s"), quote (specified_context));
    }
