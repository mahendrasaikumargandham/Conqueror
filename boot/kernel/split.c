...
static inline bool ignorable (int err)
{
  ...
  if (split_type == type_chunk_bytes || split_type == type_chunk_lines || split_type == type_rr)
    {
      ...
      if (numeric_suffix_start)
        {
          uintmax_t n_start;
          strtol_error e = xstrtoumax (numeric_suffix_start, NULL, 10, &n_start, "");
...
static bool cwrite (bool new_file_flag, char const *bp, size_t bytes)
{
  if (new_file_flag)
    {
...
static void bytes_split (uintmax_t n_bytes, char *buf, size_t bufsize, size_t initial_read, uintmax_t max_files)
{
  size_t n_read;
  ...
  do
    {
      if (initial_read != SIZE_MAX)
        ...
        {
          if (! filter_ok && lseek (STDIN_FILENO, to_write, SEEK_CUR) != -1)
            {
              to_write = n_bytes;
              new_file_flag = true;
            }
...
static void lines_chunk_split (uintmax_t k, uintmax_t n, char *buf, size_t bufsize, size_t initial_read, off_t file_size)
{
...
static bool ofile_open (of_t *files, size_t i_check, size_t nfiles)
{
      ...
      while (true)
        {
          if (files[i_check].ofd == OFD_NEW)
            ...
            {
              fd = open (files[i_check].of_name, O_WRONLY | O_BINARY | O_APPEND | O_NONBLOCK);
            }
